...
function renderHeader(task) {
  ...
  task["category"]["color"] = categoryColorCheck(category);
  let color = task["category"]["color"];
  return `<div id="user_cards" class="user_cards" draggable="true" 
      ondragstart="startDragging(${task.id})" onclick="openCardDetails(${
    task.id
  })">
        <div class="user_card_content"> <div class="cardHeader">
          ...
          class="responseDragAndDropButton" onclick="openPositionMenu(event, ${
            task.id
          })">
            <img src="../assets/img/board/dots.png" alt=""/>
            </button><div class="menuForPosition">${renderPositionMenu(
              task.id
            )}</div>
          </div>
...
function calcAssignedPersons(task) {
  ...
  for (let i = 0; i < loopValue; i++) {
    let assignedTo = contacts.find(
      (element) => element.id == task["assignedTo"][i]
    );
